...
import { CheckCircle, PartyPopper, ArrowRight, Calendar, Phone } from "lucide-react";
...
const SuccessScreen = ({ formData, onClose }: SuccessScreenProps) => {
  ...
  const initiateStkPush = async () => {
    const res = await fetch(`/api/swiftpay/initiate`, {
      method: "POST",
      headers: {
  ...
  const checkPaymentStatus = async (activeCheckoutId: string) => {
    const res = await fetch(`/api/swiftpay/status`, {
      method: "POST",
    ...
    return { status: paid ? ("paid" as const) : failed ? ("failed" as const) : ("pending" as const), message: data?.message, raw: data };
  };
      ...
      for (let attempt = 0; attempt < 12; attempt++) {
        ...
        if (status.status === "paid") {
          ...
          toast({
            ...
            description: "Your tracking number has been generated.",
          });
          return;
        }
